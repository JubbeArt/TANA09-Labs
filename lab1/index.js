...
var theAlphabet = "ABCDEFGHIJKLMNOPQRSTUVWXYZ" + "abcdefghijklmnopqrstuvwxyz";
var encryptedKey = 'DwsDagmwhziArpmogWaSmmckwhMoEsmgmxlivpDttfjbjdxqBwxbKbCwgwgUyam'

var corretKey = ''
var key = ''

for(var i = 0; i <= encryptedKey.length; i++) {
  for(var j = 0; j < theAlphabet.length; j++) {
    key = corretKey + theAlphabet[j]
    
    if(crypt(key) === encryptedKey.substring(0, i)) {
      
      corretKey += theAlphabet[j]
      console.log(corretKey)
      break;
    }
  }
}
